...
+ (NSURL *)defaultURLForImageURL:(id)imageURL {
    ...
    NSString *prefix = @"http://ss1.sinaimg.cn/orignal/";
    NSString *prefix1 = @"http://ss1.sinaimg.cn/bmiddle/";
    NSString *prefix2 = @"http://ss1.sinaimg.cn/thumbnail";
    if (link.length == 0) return nil;
    if ([link hasPrefix:prefix]) {
        NSString *sub = [link substringFromIndex:prefix.length];
        link = [@"http://ww2.sinaimg.cn/large/" stringByAppendingString:sub ];
    }else if ([link hasPrefix:prefix1]) {
        NSString *sub = [link substringFromIndex:prefix1.length];
        link = [@"http://ww2.sinaimg.cn/bmiddle/" stringByAppendingString:sub ];
    }else if ([link hasPrefix:prefix2]) {
        NSString *sub = [link substringFromIndex:prefix2.length];
        link = [@"http://ww2.sinaimg.cn/thumbnail/" stringByAppendingString:sub ];
    }
    
    if([link hasSuffix:@"&690"]){
